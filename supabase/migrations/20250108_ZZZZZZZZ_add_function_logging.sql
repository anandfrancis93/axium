...
BEGIN
  -- Count topics BEFORE filtering
  SELECT COUNT(*) INTO v_topic_count
  FROM topics
  WHERE chapter_id = p_chapter_id;

  SELECT COUNT(*) INTO v_depth_0_count
  FROM topics
  WHERE chapter_id = p_chapter_id AND depth = 0;

  SELECT COUNT(*) INTO v_depth_1_count
  FROM topics
  WHERE chapter_id = p_chapter_id AND depth = 1;

  SELECT COUNT(*) INTO v_depth_2_plus_count
  FROM topics
  WHERE chapter_id = p_chapter_id AND depth >= 2;

  RAISE NOTICE 'get_unlocked_topic_arms called - Chapter: %, Total topics: %, Depth 0: %, Depth 1: %, Depth 2+: %',
    p_chapter_id, v_topic_count, v_depth_0_count, v_depth_1_count, v_depth_2_plus_count;
...
COMMENT ON FUNCTION get_unlocked_topic_arms IS 'Get available arms with internal logging';
