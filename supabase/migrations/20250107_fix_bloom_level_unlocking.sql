-- Fix get_available_arms to properly check Bloom level prerequisites
-- Each Bloom level should only unlock after mastering the previous level
...
BEGIN
  ...
  LEFT JOIN user_progress_data upd
    ON upd.topic = tbc.topic
  ORDER BY tbc.topic, tbc.bloom_level;
END;
$$ LANGUAGE plpgsql;

COMMENT ON FUNCTION get_available_arms IS 'Get available (topic, bloom_level) arms for Thompson Sampling with proper prerequisite checking';
